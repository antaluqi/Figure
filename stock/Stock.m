classdef Stock<handle
    ...
    methods
        ...
        %--------------------------------------------------------------------------------------------------------------------------------------- 
         ...
         function Val=HistoryTick(obj,Date) %历史逐笔交易
             %-----------------------------------------读取接口数据
             url=[' http://market.finance.sina.com.cn/downxls.php?date=',Date,'&symbol=',obj.Code] 
             % 读取网页信息
             [sourcefile, status] =urlread(sprintf(url),'Charset','GBK');
             if ~status
                 error('读取错误\n')
             ...
             if ~strcmp(Val{4},'</script>') % 只要数据符合要求就转换成Table数据
                 Val=reshape(Val(1:end-1),6,(length(Val)-1)/6)';
                 Name={'Time','Price','Change','Volume','Amount','Direction'};
                 Val=cell2table([Val(end:-1:2,1),num2cell(str2double(Val(end:-1:2,2:5))),Val(end:-1:2,6)],'VariableNames',Name);
             else
                 Val=[];
             end;
         ...
         function Val=HistoryDaily2(obj,BeginDate,EndDate,XR)% 历史日线交易
             ...
             %----------------------------------------日期格式输入转换
             BeginDateF=datestr(BeginDate,'yyyymmdd');
             EndDateF=datestr(EndDate,'yyyymmdd');
             %----------------------------------------接口数据读取
             url=['http://biz.finance.sina.com.cn/stock/flash_hq/kline_data.php?symbol=',obj.Code,'&end_date=',EndDateF,'&begin_date=',BeginDateF] % 新浪K线数据
             % 读取网页信息
             [sourcefile, status] =urlread(sprintf(url),'Charset','GBK');
             if ~status
                 error('读取错误\n')
             ...
             if ~isempty(Val)
                 ...
                 %-------------------------------------加入当日变动数据
                 if datenum(EndDate)>=today && datenum(Val.Date(end,1))<today & ~strcmp(obj.State,'S')
                     obj.Flash;
                     if datenum(obj.LastTime)>=today && datenum(obj.LastTime)>datenum(Val.Date(end,1))
                         TodayK=[datestr(today,'yyyy-mm-dd'),num2cell([obj.OpenPrice,obj.High,obj.RealPrice,obj.Low,obj.Volume])];
                         Val=[Val;TodayK];
                     end
                 end
                 %---------------------------------------
                 %---------------------------------------------------------复权
                 if nargin ==4 & strcmp(XR,'L')
                     url=['http://finance.sina.com.cn/realstock/company/',obj.Code,'/qianfuquan.js?d=',datestr(today,'yyyy-mm-dd')] % 前复权接口
                     % 读取网页信息
                     [sourcefile, status] =urlread(sprintf(url),'Charset','GBK');
                     if ~status
                         error('读取错误\n')
                     end
                     date_tokens= regexp(sourcefile, '[_:,"]', 'split')';
                     y=date_tokens(5:7:end-1);
                     m=date_tokens(6:7:end-1);
                     d=date_tokens(7:7:end-1);
                     p=date_tokens(9:7:end-1);
                     d=[strcat(y,'-',m,'-',d),num2cell(str2double(p))];
                     XR=cell2table(d(end:-1:1,:),'VariableNames',{'Date','XR'});
                     LastData=XR.XR(end)*(1+obj.Yield/100);
                     if isempty(LastData)
                         LastData=NaN;
                     end
                    XR=[XR;{datestr(today,'yyyy-mm-dd'),LastData}]; %---------------

                     ValXR=innerjoin(Val,XR);
                     Val=ValXR(:,1:6);
                     ValXR.XR=ValXR.XR/ValXR.XR(end);
                     Val.Close=roundn(ValXR.XR*ValXR.Close(end),-2);
                     Val.Open=ValXR.Open+(Val.Close-ValXR.Close);
                     Val.High=ValXR.High+(Val.Close-ValXR.Close);
                     Val.Low=ValXR.Low+(Val.Close-ValXR.Close);      
                     
                 elseif nargin ==4 & strcmp(XR,'R')
                      url=['http://finance.sina.com.cn/realstock/company/',obj.Code,'/houfuquan.js?d=',datestr(today,'yyyy-mm-dd')] % 后复权接口
                       % 读取网页信息
                     [sourcefile, status] =urlread(sprintf(url),'Charset','GBK');
                     if ~status
                         error('读取错误\n')
                 ...
                 elseif nargin ==4 & ~strcmp(XR,'R') & ~strcmp(XR,'L')
                     error('除权参数应为：前复权 L 后复权 R')
                 else  
                 end
             end
         ...
         function Val=HistoryDaily1(obj,BeginDate,EndDate,XR)% 历史日线交易
             ...
             if ~isempty(Val)
                ...
                % Val=[Val(:,1),num2cell(str2double(Val(:,2:6)))]
                 Val=[datenum(Val(:,1)),str2double(Val(:,2:6))];
                 %-------------------------------------加入当日变动数据
         ...
         end % HistoryDaily(腾讯复权数据)
         function Val=HistoryDaily(obj,BeginDate,EndDate,XR)% 历史日线交易
             % e.g.    S.HistoryDaily('2015-07-03','2015-07-28')      % 不复权
             % e.g.    S.HistoryDaily('2015-','2007-0315-07-28','R') % 后复权
             % e.g.    S.HistoryDaily('2015-07-03','2015-07-28','L')  % 前复权
             %----------------------------------------日期格式输入转换
             ...
             %----------------------------------------代码转换为000001.SZ形式
             code=[obj.Code(3:end),'.',upper(obj.Code(1:2))];
             %----------------------------------------数据域
             fields='open,high,close,low,volume';
             %---------------------------------------------------------复权
             if nargin ==4 && strcmp(XR,'L')
                [w_wsd_data,w_wsd_codes,w_wsd_fields,w_wsd_times,w_wsd_errorid,w_wsd_reqid]=obj.w.wsd(code,fields,BeginDateF,EndDateF,'PriceAdj=F');
             elseif nargin ==4 && strcmp(XR,'R')
                [w_wsd_data,w_wsd_codes,w_wsd_fields,w_wsd_times,w_wsd_errorid,w_wsd_reqid]=obj.w.wsd(code,fields,BeginDateF,EndDateF,'PriceAdj=B'); 
             elseif nargin ==4 && ~strcmp(XR,'R') && ~strcmp(XR,'L')
                 error('除权参数应为：前复权 L 后复权 R')
             elseif nargin <4
                [w_wsd_data,w_wsd_codes,w_wsd_fields,w_wsd_times,w_wsd_errorid,w_wsd_reqid]=obj.w.wsd(code,fields,BeginDateF,EndDateF);  
             end
             Names={'Date','Open','High','Close','Low','Volume'};
             i=~isnan(w_wsd_data(:,end)) & (w_wsd_data(:,end)~=0);
             Times=w_wsd_times(i);
             Datas=w_wsd_data(i,:);
             %-------------------------------------加入当日变动数据
             if datenum(EndDate)>=today && datenum(Times(end))<today 
                 obj.Flash;
                 if datenum(obj.LastTime)>=today && datenum(obj.LastTime)>datenum(Times(end)) && ~strcmp(obj.State,'S')
                     Times=[Times;today];
                     Datas=[Datas;[obj.OpenPrice,obj.High,obj.RealPrice,obj.Low,obj.Volume]];
                 end
             end
             %-------------------------------------转换为table数据 
             Val=cell2table([cellstr(datestr(Times,'yyyy-mm-dd')),num2cell(Datas)],'VariableNames',Names);
         end %HistoryDaily(Wind数据)
         %---------------------------------------------------------------------------------------------------------------------------------------
         function Val=Indicators(obj, type ,ParameterList,DateRange,varargin) %计算各种指标
             ...
             elseif length(DateRange)==2
              ...
              KData=obj.HistoryDaily(st,ed,'L'); % K线数据选择前复权
              L=length(KData.Date);
              for i=1:length(type)
                    switch type{i}
                        case 'Y' % 收益率
    ...
    methods (Static) 
        ...
        function info=Handicap(Code)%盘口信息
            ...
            Name={'Market','Name','Code','RealPrice','YClosePrice','OpenPrice','Volume','B','S','Buy1Price','Buy1Volume','Buy2Price','Buy2Volume','Buy3Price','Buy3Volume','Buy4Price','Buy4Volume','Buy5Price','Buy5Volume','Sell1Price','Sell1Volume','Sell2Price','Sell2Volume','Sell3Price','Sell3Volume','Sell4Price','Sell4Volume','Sell5Price','Sell5Volume','LastTransaction','LastTime','Rise','Yield','HighPrice','LowPrice','Price_Volume_Amount','Volume2','Amount','HSL','PE','State','High','Low','Amplitude','CirculationMarketValue','TotalMarketValue','PB','HardenPrice','LimitPrice','blank2'};
        ...
        function info=Fund(Code) %资金流向
            ...
            Explain={'代码','主力流入','主力流出','主力净流入','主力净流入/资金流入流出总和','散户流入','散户流出','散户净流入','散户净流入/资金流入流出总和',...
             ...
             Name={'Code','MInflow','MOutflow','MNetInflow','MNetInflow_Total','RInflow','ROutflow','RNetInflow','RNetInflow_Total','TotalFund','unknow1','unknow2','Name','LastTime','unknow3','unknow4','unknow5','unknow6'};
             if ~isempty(S)
                 info=cell2table(S,'VariableNames',Name);
             else
                 info=[];
